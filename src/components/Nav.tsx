...
export default function Nav() {
  ...
  return (
    <>
      ...
      `}</style>
      <nav style={{ position: "fixed", top: "4px", left: 0, right: 0, zIndex: 100, background: "rgba(247,244,239,0.94)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--line-faint)" }}>
        <div className="nav-bar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.4rem 5rem 1.4rem 6.5rem" }}>
          <a href="/" style={{ fontFamily: "'Shippori Mincho', serif", fontSize: "1.1rem", color: "var(--ink)", letterSpacing: "0.08em", fontWeight: 400, textDecoration: "none" }}>
            Ankina Lab
          </a>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            <div className="nav-links">
              {navItems.map((item) => (
                <a key={item.href} href={item.href} style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--muted)", textDecoration: "none", letterSpacing: "0.16em", textTransform: "uppercase" }}>
                  {item.label}
                </a>
              ))}
            </div>
            <button onClick={handleLangSwitch} style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--anko-mid)", background: "transparent", border: "1px solid rgba(58,28,82,0.2)", padding: "0.2rem 0.6rem", letterSpacing: "0.1em", cursor: "pointer", whiteSpace: "nowrap" }}>
              {lang === "en" ? "日本語" : "EN"}
            </button>
        ...
        <div className={`nav-mobile${menuOpen ? " open" : ""}`}>
          {navItems.map((item) => (
            <a key={item.href} href={item.href} onClick={() => setMenuOpen(false)} style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.75rem", color: "var(--muted)", textDecoration: "none", letterSpacing: "0.16em", textTransform: "uppercase" }}>
              {item.label}
            </a>
          ))}
        </div>
